...
sudo apt-fast install git terminator tmux zim openjdk-11-jdk cmake curl htop mlocate lm-sensors powertop iproute2 inetutils-ping r-base-core gdebi-core groovy xclip sysbench python3-pip python-dev net-tools -y
sudo apt-fast install zsh gnome-tweaks -y  # sudo chsh -s /bin/zsh tchr
